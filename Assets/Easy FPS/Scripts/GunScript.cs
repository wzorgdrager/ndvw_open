...
public class GunScript : MonoBehaviour {
	...
	*/
	void FixedUpdate(){
		 ...
		 */
		//if aiming
		if(Input.GetMouseButton(1) && !reloading && !meeleAttack){
	...
	void CrossHairExpansionWhenWalking(){
		...
		if(player.GetComponent<Rigidbody>().velocity.magnitude > 1 && !Input.GetMouseButton(0)){//ifnot shooting
	...
	void Sprint(){// Running();  so i can find it with CTRL + F
		if (Input.GetAxis ("Vertical") > 0 && !Input.GetMouseButton(0) && meeleAttack == false && !Input.GetMouseButton(1)) {
	...
	*/
	...
	void DrawCrosshair(){
		...
		if(!Input.GetMouseButton(1)){//if not aiming draw
	...
	*/
	void Animations(){
		...
		if(handsAnimator){
			...
			handsAnimator.SetBool("aiming", Input.GetMouseButton(1));
